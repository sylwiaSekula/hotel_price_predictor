...
def main():
    ...
    pickle.dump(encoder, open(os.path.join(trained_model_dir, ordinal_encoder_file), 'wb'))
    ...
    rf_results = cross_validate(rf_regressor_opt, X_train_scaled, y_train, cv=kf, scoring=scoring)
    xgb_results = cross_validate(xgb_opt, X_train_scaled, y_train, cv=kf, scoring=scoring)
    knn_results = cross_validate(knn_opt, X_train_scaled, y_train, cv=kf, scoring=scoring)
